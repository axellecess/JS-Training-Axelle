...
export function calcTotalPrice(products) {
  // return total price ofupall products (unitPrice * quantitpy)
  const productPrices = products.map((p) => p.quantity * p.unitPrice);
  return productPrices.reduce((sum, n) => sum + n);
  //return products.reduce((tot, p) => tot + (p.unitPrice * p.quantity));
}
